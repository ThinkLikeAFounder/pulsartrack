...
import { isSensitiveKey, scrub, scrubEvent, scrubString } from "../lib/sentry-scrubber";
...
describe("Sentry Scrubber", () => {
  ...
  it("should keep backend and frontend sentry-scrubber files in sync", () => {
    const backendPath = path.resolve(__dirname, "../lib/sentry-scrubber.ts");
    const frontendPath = path.resolve(__dirname, "../../../frontend/src/lib/sentry-scrubber.ts");
    const backendContent = fs.readFileSync(backendPath, "utf-8");
    const frontendContent = fs.readFileSync(frontendPath, "utf-8");
    expect(backendContent).toBe(frontendContent);
  });
});

...
const REFUND_GRACE_PERIOD: u64 = 86_400; // 24 hours
...
impl TokenBridgeContract {
    ...
    pub fn deposit_for_bridge(
        ...
        let expires_at = created_at.saturating_add(REFUND_GRACE_PERIOD);
    ...
    pub fn refund_deposit(env: Env, admin: Address, deposit_id: u64) {
        ...
        let total_refund = deposit.original_amount;
    ...
    pub fn refund_expired(env: Env, sender: Address, deposit_id: u64) {
        env.storage()
            .instance()
            .extend_ttl(INSTANCE_LIFETIME_THRESHOLD, INSTANCE_BUMP_AMOUNT);
        sender.require_auth();
        ...
        if env.ledger().timestamp() < deposit.expires_at {
            panic!("refund not available yet");
        }
